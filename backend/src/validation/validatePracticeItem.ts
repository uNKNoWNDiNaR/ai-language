...
const GRADING_CONTAMINATION_PATTERNS: RegExp[] = [
...
function hasGradingContamination(text: string): boolean {
    return GRADING_CONTAMINATION_PATTERNS.some((re) => re.test(text));
}

export function validatePracticeItem(input: unknown): ValidationOk<PracticeItem> | validationErr {
    const errors: string[] = [];
    ...
    if(!isNonEmptyString(practiceId)) errors.push("Practice Id is required");
    if(!isNonEmptyString(lessonId)) errors.push("lessonId is required");
    ...
    if(!isNonEmptyString(prompt)) errors.push("prompt is required");
    ...
    if(!isRecord(meta)) {
        ...
        } else if (!SUPPORTED_TYPES.includes(metaType as PracticeMetaType)) {
    ...
    if(examples !== undefined) {
        if(!Array.isArray(examples)) {
            ...
            if(anyInvalid) errors.push("examples may not contain empty strings");
            if(examples.length > MAX_EXAMPLES) errors.push(`examples must be at most ${MAX_EXAMPLES}`);
        }
    }
    ...
    if(hints !== undefined) {
        if(!Array.isArray(hints)) {
            ...
            const anyInvalid = cleaned.some((x) => x.length === 0);
            if(anyInvalid) errors.push("hints may not contain empty strings");
        }
    }

    if(errors.length > 0) return {ok: false, errors};

    return {ok: true, value: input as PracticeItem};
}
