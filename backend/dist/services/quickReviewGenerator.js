...
exports.isPromptLeakingAnswer = isPromptLeakingAnswer;
exports.generateQuickReviewItems = generateQuickReviewItems;
const mapLike_1 = require("../utils/mapLike");
const SMART_APOSTROPHES = /[’‘]/g;
function normalizeLeak(text) {
    return (text || "")
        .toLowerCase()
...
const GENERIC_PROMPT_RE = /^(write|make)\s+(a\s+)?(short\s+)?sentence\.?$/i;
function isPromptLeakingAnswer(prompt, answer) {
...
function isGenericPrompt(prompt) {
    ...
    if (!cleaned)
        return true;
    if (GENERIC_PROMPT_RE.test(cleaned))
        return true;
    return cleaned === "short response";
}
function hasMeaningfulContext(prompt) {
    const cleaned = (prompt || "")
        .replace(/_+/g, " ")
...
function pickQuestionPrompt(question) {
...
function getKind(question) {
    const prompt = typeof question.prompt === "string" ? question.prompt : "";
    if (question.expectedInput === "blank")
        return "blank";
    if (question.promptStyle === "BLANK_AUX")
        return "blank";
    if (prompt.includes("___"))
        return "blank";
    if (question.promptStyle === "WORD_BANK")
        return "word_bank";
    return "short_answer";
}
function getAttemptCount(attempts, questionId) {
    if (!attempts)
        return 0;
    return (0, mapLike_1.mapLikeGetNumber)(attempts, String(questionId), 0);
}
function buildBlankPrompt(question, promptOverride) {
    ...
    if (prompt && prompt.includes("___")) {
        ...
        if (blanks.length === 0)
            return null;
        if (!hasMeaningfulContext(prompt))
            return null;
    ...
    if (!hasMeaningfulContext(finalPrompt))
        return null;
...
function buildWordBankPrompt(answer) {
...
function pickHintContext(question) {
    const hintTarget = typeof question.hintTarget === "string" ? question.hintTarget.trim() : "";
    const explanationTarget = typeof question.explanationTarget === "string"
        ? question.explanationTarget.trim()
        : "";
    const hintSupport = typeof question.hintSupport === "string" ? question.hintSupport.trim() : "";
    const explanationSupport = typeof question.explanationSupport === "string"
        ? question.explanationSupport.trim()
        : "";
    const hintLegacy = typeof question.hint === "string" ? question.hint.trim() : "";
    const hints = Array.isArray(question.hints)
        ? question.hints
...
function isHumanConceptTag(conceptTag) {
...
function buildConceptPrompt(conceptTag) {
    if (!isHumanConceptTag(conceptTag))
        return "";
...
function buildMaskedTokenPrompt(answer) {
    const tokens = extractTokens(answer);
    if (tokens.length === 0)
        return "";
...
function buildSpecificSentencePrompt(question, answer, conceptTag) {
...
function buildMicroItem(candidate, lessonId) {
...
function toPracticeItem(item, lessonId, language, question) {
...
function selectCandidates(candidates, targetCount) {
    const sorted = [...candidates].sort(sortCandidates);
    if (targetCount <= 1)
        return sorted.slice(0, targetCount);
    const kindTop = new Map();
    for (const cand of sorted) {
        if (!kindTop.has(cand.kind))
            kindTop.set(cand.kind, cand);
    }
    const topKinds = Array.from(kindTop.values()).sort(sortCandidates);
    const desiredKinds = topKinds.length >= 2 && targetCount >= 2 ? [topKinds[0].kind, topKinds[1].kind] : [];
    const used = new Set();
    const selected = [];
...
function generateQuickReviewItems(input) {
    ...
    for (const question of lesson.questions ?? []) {
        ...
        const candidate = { question, conceptTag, attemptCount, kind };
    ...
    if (items.length < 2 && candidates.length > items.length) {
        ...
        for (const cand of remaining) {
            if (items.length >= Math.min(maxItems, candidates.length))
                break;
