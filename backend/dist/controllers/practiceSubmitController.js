...
function getFeedbackStrings(language) {
    if (language === "de") {
        return {
            correct: "Gut gemacht.",
            almost: "Fast.",
...
function getHintForAttempt(item, attemptCount, language) {
...
function buildTutorMessage(result, hint, language) {
    ...
    if (result === "almost")
        return hint ? `${strings.almost} ${hint}` : `${strings.almost} ${strings.tryAgain}`;
